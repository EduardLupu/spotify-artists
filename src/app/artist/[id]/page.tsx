...
export default function ArtistPage() {
  ...
  useEffect(() => {
    const artistId = params?.id as string | undefined
    if (!artistId) return
        ...
        <header className="border-b border-white/10 bg-black/40 backdrop-blur-xl">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-6">
            ...
            <div className="flex items-center gap-3 text-sm text-white/70">
              <div className="flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-emerald-200">
        ...
        <main className="mx-auto max-w-6xl px-4 pb-24">
          <section className="relative mt-10 overflow-hidden rounded-3xl border border-white/10 bg-black/40">
            <div className="absolute inset-0">
                  ...
                  {galleryImages.map((url, index) => (
                    ...
                    >
                      <img
                        ...
                        className="h-full w-full object-cover transition-transform duration-[7000ms] ease-linear will-change-transform group-hover:scale-105"
                        onError={(event) => {
              ...
              <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/70 to-black/80 backdrop-blur-3xl" />
            </div>
            ...
            <div className="relative z-10 flex flex-col gap-8 px-6 py-10 md:flex-row md:items-center md:justify-between md:px-10">
              <div className="flex flex-1 flex-col gap-6 md:flex-row md:items-center">
                <div className="relative shrink-0">
                  <div className="absolute -left-6 -top-6 h-24 w-24 rounded-full bg-emerald-500/30 blur-3xl" />
                  <img
                    src={heroImage}
                    alt={artist.n}
                  ...
                  />
                  {artist.today.r && (
                    <Badge className="absolute -bottom-3 left-1/2 -translate-x-1/2 rounded-full border-none bg-emerald-400/90 px-4 py-1 text-xs font-semibold uppercase tracking-widest text-emerald-950 shadow-lg shadow-emerald-500/30">
                      #{artist.today.r}
                    </Badge>
                  )}
                </div>
                <div className="space-y-4">
                  <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-1 text-xs uppercase tracking-[0.4em] text-white/60">
                  ...
                  <p className="max-w-xl text-sm text-white/70">
                    Real-time analytics aggregated from Spotify. Momentum combines streak health, growth velocity, and catalogue freshness.
                  </p>
                ...
                <div className="flex w-max gap-4 pb-3 pr-4">
                  {galleryImages.map((url, index) => (
                    ...
                    >
                      ...
                      />
                      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/40 opacity-60" />
                    </div>
                  ))}
                </div>
              </ScrollArea>
          ...
          <section className="mt-6 grid gap-4 sm:grid-cols-3">
            {growthMetrics.map(({ label, value, icon: Icon }) => (
              <Card key={label} className="border-white/10 bg-black/40">
                ...
                <CardContent>
                  <div className={`text-xl font-semibold ${value.startsWith('-') ? 'text-rose-200' : 'text-emerald-200'}`}>
                    {value}
                  </div>
                </CardContent>
              </Card>
          ...
          <section className="mt-10 grid gap-6 lg:grid-cols-[2.1fr_1fr]">
            <Card className="border-white/10 bg-black/40">
              <CardHeader className="flex flex-col gap-1">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs uppercase tracking-widest text-white/60">
            ...
            <Card className="border-white/10 bg-white/5 backdrop-blur">
              ...
              <CardContent className="space-y-4 text-sm text-white/80">
                {metaRows.map(({ label, value }) => (
                  <div key={label} className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/30 px-4 py-3">
                    ...
                    <div className="flex flex-col gap-3 pb-1">
                      {topTracks.map((track, index) => {
                        ...
                        return (
                          <div
                            key={track.id}
                            className="group relative flex items-center gap-4 overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-3 transition-all duration-200 hover:border-emerald-400/40 hover:bg-emerald-400/5"
                          >
                            ...
                            <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-white/10">
                              ...
                              />
                              {isActive && (
                                <div className="absolute inset-0 flex items-center justify-center bg-black/60 text-emerald-300">
                              ...
                              <div className="flex flex-wrap items-baseline justify-between gap-2">
                                <div>
                                  <div className="flex items-center gap-2">
                                    <span className="rounded-full border border-white/10 bg-black/40 px-2 py-0.5 text-[10px] uppercase tracking-widest text-white/60">
                                      #{index + 1}
                                    </span>
                                  ...
                                  <p className="mt-1 text-[11px] uppercase tracking-wide text-white/50">
                                    {track.label ?? 'Independent'} • {track.language?.toUpperCase() || 'LANG'}
                                  </p>
                                </div>
                                <div className="text-right text-xs text-white/60">
                                  ...
                                  <p>Released {formatDate(track.releaseDate)}</p>
                                </div>
                              </div>
              ...
              <Card className="border-white/10 bg-white/5 backdrop-blur">
                ...
                <CardContent className="space-y-4 text-sm text-white/70">
                  <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3">
                    <p className="text-xs uppercase tracking-widest text-white/50">Current streak</p>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3">
                    <p className="text-xs uppercase tracking-widest text-white/50">Best global rank</p>
                    <p className="mt-1 text-lg font-semibold text-white">
                      {artist.meta.br ? `#${artist.meta.br}` : '—'}
                    </p>
                    <p className="text-xs text-white/50">Rank delta today {formatRankDelta(artist.today.dr)}</p>
                  </div>
                </CardContent>
              </Card>
            </section>
